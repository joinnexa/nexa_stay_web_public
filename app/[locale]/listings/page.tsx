...
function ListingCard({ listing, checkin, checkout, guests, city, t, localePath }: { listing: StaysListing; checkin?: string; checkout?: string; guests?: number; city?: string; t: (k: string) => string; localePath: (p: string) => string }) {
  ...
  const linkHref = localePath(`/listings/${listing.id}${detailUrl.toString() ? `?${detailUrl}` : ""}`);

  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-nexa-line/60 transition-all duration-300 hover:shadow-xl hover:border-nexa-line hover:-translate-y-0.5 group">
      <Link href={linkHref} className="block relative">
        <ListingImageGallery
          ...
          showArrows={photoCount > 1}
          showDots={photoCount > 1}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent pointer-events-none" />
        <div className="absolute top-3 left-3 right-3 flex justify-between items-start">
          <span className="inline-flex px-2.5 py-1 rounded-lg text-[0.7rem] font-semibold uppercase tracking-wide bg-white/95 text-nexa-ink shadow-sm">
            {listing.listing_type}
          </span>
          <button
            type="button"
            className="w-9 h-9 rounded-full bg-white/95 flex items-center justify-center text-nexa-ink-4 hover:text-nexa-primary hover:bg-white shadow-sm transition-colors"
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-center gap-2">
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[0.7rem] font-medium bg-white/95 text-nexa-ink shadow-sm">
            {hasWalkthrough ? "🎬" : "✓"} {verified}
          </span>
          {(photoCount > 0 || hasWalkthrough) && (
            <span className="text-[0.7rem] text-white/90 drop-shadow-sm">
              {photoCount} photo{photoCount !== 1 ? "s" : ""}{hasWalkthrough ? " · Video" : ""}
            </span>
          )}
        </div>
      </Link>
      <div className="p-5">
        <p className="text-[0.75rem] font-medium text-nexa-ink-4 uppercase tracking-wider mb-1">{listing.city}</p>
        <h3 className="font-semibold text-[1.05rem] text-nexa-ink mb-1.5 line-clamp-1">{listing.title}</h3>
        <p className="text-[0.875rem] text-nexa-ink-3 line-clamp-2 mb-4 min-h-[2.5rem]">
          {listing.description || `${listing.listing_type} in ${listing.city}`}
        </p>
        <div className="flex items-center justify-between pt-3 border-t border-nexa-line/60">
          <div>
            <span className="font-bold text-lg text-nexa-ink">{price}</span>
            <span className="text-sm text-nexa-ink-4 ml-1">{(listing.rate_plan?.currency || "MAD")}/night</span>
          </div>
          <Button size="sm" asChild className="rounded-xl font-medium">
            <Link href={linkHref}>{t("listings.viewStay")}</Link>
          </Button>
        </div>
        <p className="text-[0.7rem] text-nexa-ink-4 text-center pt-3 mt-3 border-t border-nexa-line/40">
          ...
          <div className="bg-nexa-bg">
            <div className="bg-white border-b border-nexa-line py-3 sm:py-4 px-4 sm:px-6 md:px-8 flex flex-wrap items-center gap-3 sm:gap-4 sticky top-[72px] z-10">
              ...
              >
                ...
                <input type="date" name="checkin" defaultValue={checkin} min={new Date().toISOString().split("T")[0]} className="h-11 rounded-xl border border-nexa-line px-3 py-2 text-sm" />
                <input type="date" name="checkout" defaultValue={checkout} min={checkin || new Date().toISOString().split("T")[0]} className="h-11 rounded-xl border border-nexa-line px-3 py-2 text-sm" />
              ...
              <div className="bg-nexa-primary-soft border border-nexa-primary/15 rounded-[22px] p-6 px-7 flex items-center gap-8">
                ...
                <div className="flex gap-6 flex-wrap">
                  {["listings.verifiedWalkthrough", "listings.verifiedIdentity", "listings.accurateLocation", "listings.clearCheckinContact"].map((key) => (
                    <div key={key} className="flex items-center gap-2 text-sm text-nexa-ink-3">
