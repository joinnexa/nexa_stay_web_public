...
export function LanguageSelector() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div
          className="absolute right-0 top-full mt-1 py-1 min-w-[140px] bg-white rounded-lg shadow-lg border border-nexa-line z-50"
          role="menu"
        >
          {LOCALES.map(({ code, name, native }) => (
            ...
            >
              ...
              {native !== name && (
                <span className="ml-1.5 text-nexa-ink-4 text-xs">({name})</span>
              )}
            </button>
          ))}
        </div>
